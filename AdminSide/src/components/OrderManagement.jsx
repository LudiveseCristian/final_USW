    ...
    const unsubscribe = onSnapshot(collection(db, 'products'), async (snapshot) => {
      ...
      snapshot.docs.forEach((d) => {
        ...
        if (data.status === 'sold' || data.winnerBidderId) {
          ...
          const fetchOrderDetails = async () => {
            ...
            if (data.orderId) {
              try {
                ...
                if (orderDocSnap.exists()) {
                  deliveryAddress = orderDocSnap.data().deliveryAddress || null;
                }
              } catch (err) {
                console.error(`Failed to fetch order details for order ${data.orderId}:`, err);
              }
            }
            ...
            if (winnerBid) {
              return {
                ...
                orderDate: data.orderDate || data.soldAt || data.createdAt || null,
          ...
          soldProductPromises.push(fetchOrderDetails());
        }
      });
                      ...
                      {(order.orderStatus === 'pending' || order.orderStatus === 'shipped') ? (
                        ...
                        order.userRating ? (
                          <div className="flex flex-col space-y-1">
                            ...
                            <div className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 border border-purple-200 w-fit">
                              <Star className="w-3 h-3 mr-1" />
                              Rated
                            </div>
                          </div>
                        ) : (
                          <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor('delivered')}`}>
                            {getStatusIcon('delivered')}
                            <span className="ml-2">Complete (Unrated)</span>
                          </div>
                        )
                      )}
                    </td>
                      ...
                      <div className="flex flex-col space-y-2">
                        ...
                        {order.orderStatus === 'delivered' && (
                            <Button
                                ...
                                size="sm"
                            >
                                <ThumbsUp className="w-4 h-4 mr-2" /> {order.userRating ? 'Complete Order' : 'Complete (No Rating)'}
                            </Button>
                        )}
                    ...
                    <div className='pt-2'>
                      {(selectedOrder.orderStatus === 'pending' || selectedOrder.orderStatus === 'shipped') ? (
                        ...
                        selectedOrder.userRating ? (
                          ...
                          <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor('delivered')}`}>
                            {getStatusIcon('delivered')}
                            <span className="ml-2">Complete (Unrated)</span>
                          </div>
                        )
                  ...
                  <div className='pt-2'>
                    ...
                    {selectedOrder.deliveryAddress ? (
                        <div className='text-gray-600 mt-1 space-y-1'>
                            <div className='text-gray-800 font-medium'>{selectedOrder.deliveryAddress.fullName || 'N/A'}</div>
                            <div>{selectedOrder.deliveryAddress.streetAddress}, {selectedOrder.deliveryAddress.barangay}</div>
                            <div>{selectedOrder.deliveryAddress.city}, {selectedOrder.deliveryAddress.province} {selectedOrder.deliveryAddress.postalCode}</div>
                            {selectedOrder.deliveryAddress.landmark && (
                                <div className='italic text-xs'>Landmark: {selectedOrder.deliveryAddress.landmark}</div>
                            )}
                            ...
                            {selectedOrder.deliveryAddress.deliveryNotes && (
                                <div className='italic text-xs text-red-600'>Notes: {selectedOrder.deliveryAddress.deliveryNotes}</div>
                            )}
                        </div>
                    ) : (
                        <span className='text-gray-600'> No delivery address provided.</span>
                    )}
                  </div>
