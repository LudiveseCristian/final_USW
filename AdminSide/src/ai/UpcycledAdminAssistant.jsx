...
const UpcycledAdminAssistant = () => {
  ...
  const loadAdminStats = async () => {
    try {
      ...
      // Recent customers (last 7 days - assuming 'joinDate' is a Timestamp)
      const recentCustomersQuery = query(
        collection(db, 'users'),
        where('joinDate', '>=', Timestamp.fromDate(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))),
      ...
      ordersSnap.docs.forEach(doc => {
        ...
        // --- FIX: Use 'date' field, not 'createdAt' ---
        const orderDate = orderData.date?.toDate ? orderData.date.toDate() : new Date(orderData.date);
        
        // Monthly revenue
        if (orderDate >= startOfMonth) {
          // --- FIX: Use 'price' field, not 'total' ---
          monthlyRevenue += orderData.price || 0;
        }
    ...
    try {
      ...
      if (lowerQuery.includes('order') || lowerQuery.includes('claim') || lowerQuery.includes('sale')) {
        const ordersQuery = query(
          ...
          // --- FIX: Order by 'date', not 'createdAt' ---
          orderBy('date', 'desc'),
          limit(10)
        );
        const ordersSnap = await getDocs(ordersQuery);
      ...
      if (lowerQuery.includes('customer') || lowerQuery.includes('user')) {
        const customersQuery = query(
          collection(db, 'users'),
          // --- FIX: Order by 'joinDate' (to match schema notes) ---
          orderBy('joinDate', 'desc'),
          limit(10)
        );
        const customersSnap = await getDocs(customersQuery);
      ...
      // --- REMOVED: 'low stock' query ---
      // This query was based on a 'quantity' field, but your schema notes
      // indicate products are 'available' or 'sold', not quantity-based.
      // The 'availableItems' stat from loadAdminStats is the correct metric.
...
Respond as the admin assistant with specific data-driven insights:`;
  ...
  const quickAdminActions = [
    ...
    { text: "Show inventory summary (available vs. sold)", icon: <Package size={16} /> },
